...
Shader "Retus001/CellShaded" {
    Properties {
        ...
        _LightColor ("LightColor", Color) = (0.8018868,0.8018868,0.8018868,1)
        _ShadowColor ("ShadowColor", Color) = (0.3867925,0.3867925,0.3867925,1)
        ...
        _pannSpeed ("pannSpeed", Range(-5, 5)) = -5
        [MaterialToggle] _PannU ("PannU", Float ) = 0
        [MaterialToggle] _PannV ("PannV", Float ) = 0
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            uniform fixed _PannU;
            uniform fixed _PannV;
            struct VertexInput {
                float4 vertex : POSITION;
                float3 normal : NORMAL;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_5629 = _Time;
                float node_1086 = (node_5629.r*_pannSpeed);
                float2 UVPann = ((lerp( i.uv0, (i.uv0+node_1086*float2(1,0)), _PannU )+lerp( i.uv0, (i.uv0+node_1086*float2(0,1)), _PannV ))-i.uv0);
                float2 node_1855 = UVPann;
                float3 _NormalTex_var = UnpackNormal(tex2D(_NormalTex,TRANSFORM_TEX(node_1855, _NormalTex)));
...
////// Emissive:
                ...
                float3 node_9548 = lerp((node_9548_if_leA*_ShadowColor.rgb)+(node_9548_if_leB*_LightColor.rgb),_LightColor.rgb,node_9548_if_leA*node_9548_if_leB);
                float2 node_4606 = UVPann;
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(node_4606, _MainTex));
                float3 Diffuse = (_MainColor.rgb*_MainTex_var.rgb);
                float3 finalColor = emissive + (_LightColor0.rgb*node_9548*attenuation*Diffuse);
...
////// Lighting:
                ...
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(node_4606, _MainTex));
                float3 Diffuse = (_MainColor.rgb*_MainTex_var.rgb);
                float3 finalColor = (_LightColor0.rgb*node_9548*attenuation*Diffuse);
